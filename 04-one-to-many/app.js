...
function getUseryDetailByUID(userId, roomId, nickname, pub) {
...
async function onListener(socket) {
  ...
  let pub = getParam(url, 'pub') || 'no'
  console.log(userId, roomId, nickname, pub)

  userMap.set(userId, socket)
  if (roomId) {
    console.log(getUseryDetailByUID(userId, roomId, nickname, pub))
    await redisClient.hSet(roomKey + roomId, userId + '', getUseryDetailByUID(userId, roomId, nickname, pub))
