...
function getUseryDetailByUID(userId, roomId) {
...
async function onListener(socket) {
  ...
  console.log(url, userId, roomId)

  userMap.set(userId, socket)
  if (roomId) {
    await redisClient.hSet(roomKey + roomId, userId + '', getUseryDetailByUID(userId, roomId))
  ...
  socket.on('disconnect', async () => {
    ...
    // userMap.delete(roomKey + roomId, userId)
  ...
  socket.on('clean', async (data) => {
    console.log('c: ', data)
    await redisClient.del(roomKey + data['roomId'])
    await redisClient.del(`chat_` + data['roomId'])
  ...
  socket.on('chat', async (data) => {
    await redisClient.lPush(`chat_` + roomId, JSON.stringify(data))
    const list = await redisClient.lRange(`chat_` + roomId, 0, 100000)
    oneToMany(roomId, getMsg('chat', 'chat', 200, list.map(e => JSON.parse(e))))
  })
}
